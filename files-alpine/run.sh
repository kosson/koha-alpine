#!/bin/bash
set -e
# PATH must include sbin for adduser/addgroup on Alpine
...
KOHADEVBOX_DEFAULT=/kohadevbox/koha
KOHADEVBOX=${KOHADEVBOX:-$KOHADEVBOX_DEFAULT}
DB_HOST=${DB_HOST:-db}
...
# Auto-detect real koha source location (dev bind-mount vs baked-in)
detect_koha_src() {
  for cand in "$KOHADEVBOX" /kohadevbox/koha /kohadevbox /opt/koha /usr/share/koha /kohadevbox/koha/koha; do
    if [ -f "$cand/app.psgi" ] && [ -f "$cand/installer/data/mysql/kohastructure.sql" ]; then
      echo "$cand"
      return 0
    fi
  done
...
echo "[run.sh] BEEFY v20 FINAL - ${KOHASITE} MEMCACHED=${MEMCACHED_SERVERS}"
echo "[run.sh] KOHADEVBOX=$KOHADEVBOX"
ls -l ${KOHADEVBOX}/app.psgi ${KOHADEVBOX}/installer/data/mysql/kohastructure.sql 2>&1 | head -5 || echo "[run.sh] WARNING: koha source not found at $KOHADEVBOX - check SYNC_REPO in env/.env"

# Fix hardcoded /usr/share/koha path used by C4::Installer
mkdir -p /usr/share/koha/intranet/cgi-bin/installer/data/mysql
ln -sf ${KOHADEVBOX}/installer /usr/share/koha/intranet/cgi-bin/installer 2>/dev/null || true
if [ ! -d /usr/share/koha/intranet/cgi-bin/installer/data/mysql/db_revs ]; then
  rm -rf /usr/share/koha/intranet/cgi-bin/installer
  mkdir -p /usr/share/koha/intranet/cgi-bin
  ln -sf ${KOHADEVBOX}/installer /usr/share/koha/intranet/cgi-bin/installer
fi

# Ensure instance user exists (Alpine busybox + shadow compatible)
if ! getent passwd kohadev-koha >/dev/null 2>&1; then
  echo "[run.sh] Creating user kohadev-koha"
  addgroup -S kohadev-koha 2>&1 || addgroup kohadev-koha 2>&1 || groupadd kohadev-koha 2>&1 || true
  adduser -S -D -H -h ${KOHA_HOME} -G kohadev-koha -s /bin/sh kohadev-koha 2>&1 || adduser -D -h ${KOHA_HOME} -G kohadev-koha kohadev-koha 2>&1 || useradd -d ${KOHA_HOME} -g kohadev-koha -s /bin/sh kohadev-koha 2>&1 || true
  if ! getent passwd kohadev-koha >/dev/null 2>&1; then
    echo "[run.sh] Manual /etc/passwd injection"
    echo "kohadev-koha:x:1001:1001:Koha:${KOHA_HOME}:/bin/sh" >> /etc/passwd
    if ! getent group kohadev-koha >/dev/null 2>&1; then
      echo "kohadev-koha:x:1001:" >> /etc/group
    fi
  fi
fi
id kohadev-koha || echo "[run.sh] ERROR: user creation failed"
addgroup kohadev kohadev-koha 2>/dev/null || adduser kohadev kohadev-koha 2>/dev/null || usermod -aG kohadev-koha kohadev 2>/dev/null || true
...
# Ensure debian templates are present in /etc/koha
if [ ! -f /etc/koha/koha-conf-site.xml.in ]; then
  echo "[run.sh] Restoring debian templates to /etc/koha"
  cp ${KOHADEVBOX}/debian/templates/* /etc/koha/ 2>/dev/null || cp /kohadevbox/koha/debian/templates/* /etc/koha/ 2>/dev/null || true
fi

# Generate koha-conf.xml from REAL template
echo "[run.sh] Generating koha-conf.xml from REAL template"
TEMPLATE="/etc/koha/koha-conf-site.xml.in"
if [ ! -f "$TEMPLATE" ]; then
  TEMPLATE="${KOHADEVBOX}/debian/templates/koha-conf-site.xml.in"
fi
...
ZEBRA_PWD=$(head -c 32 /dev/urandom | tr -dc 'a-zA-Z0-9' | head -c16)
API_SECRET=$(head -c 64 /dev/urandom | tr -dc 'a-zA-Z0-9' | head -c32)
sed -i \
...
chmod 644 "${KOHACONF}"
chown kohadev-koha:kohadev-koha "${KOHACONF}" 2>/dev/null || chown 1001:1001 "${KOHACONF}" 2>/dev/null || true
echo "[run.sh] config size: $(wc -c < ${KOHACONF}) bytes"
grep -m1 memcached "${KOHACONF}" || echo "no memcached line"
...
chown kohadev-koha:kohadev-koha /etc/koha/sites/${KOHASITE}/log4perl.conf 2>/dev/null || true

echo "[run.sh] Waiting DB..."
for i in $(seq 1 60); do
  mysql -h ${DB_HOST} -u ${DB_USER} -p${DB_PASS} ${DB_NAME} -e "SELECT 1" >/dev/null 2>&1 && { echo "DB ready $i"; break; }
  sleep 2
done

# DB import if empty
if ! mysql -h ${DB_HOST} -u ${DB_USER} -p${DB_PASS} ${DB_NAME} -e "SELECT 1 FROM systempreferences LIMIT 1" >/dev/null 2>&1; then
  echo "[run.sh] DB empty, importing from $KOHADEVBOX"
  mysql -h ${DB_HOST} -u ${DB_USER} -p${DB_PASS} ${DB_NAME} < ${KOHADEVBOX}/installer/data/mysql/kohastructure.sql 2>&1 | tail -5 || true
  for f in ${KOHADEVBOX}/installer/data/mysql/mandatory/*.sql; do
    echo "[run.sh] importing $f"
    mysql -h ${DB_HOST} -u ${DB_USER} -p${DB_PASS} ${DB_NAME} < "$f" 2>&1 | grep -v "Duplicate" | tail -1 || true
  done
  ...
  echo "[run.sh] DB already has systempreferences, skipping import"
fi
...
echo "[run.sh] Running updatedatabase.pl as kohadev-koha"
su kohadev-koha -s /bin/sh -c "export KOHA_CONF=${KOHACONF} PERL5LIB=/opt/koha-perl/lib/perl5:${KOHADEVBOX}/lib:${KOHADEVBOX} KOHA_HOME=${KOHA_HOME}; cd ${KOHADEVBOX}; perl -Ilib installer/data/mysql/updatedatabase.pl" 2>&1 | tail -40 || true

echo "[run.sh] Starting Plack :5000 as kohadev-koha from $KOHADEVBOX"
exec su kohadev-koha -s /bin/sh -c "export KOHA_CONF=${KOHACONF} LOG4PERL_CONF=/etc/koha/sites/${KOHASITE}/log4perl.conf PERL5LIB=/opt/koha-perl/lib/perl5:${KOHADEVBOX}/lib:${KOHADEVBOX} KOHA_HOME=${KOHA_HOME} PATH=/opt/koha-perl/bin:/usr/local/bin:/usr/bin:/bin; cd ${KOHADEVBOX}; exec perl /opt/koha-perl/bin/plackup --port 5000 --host 0.0.0.0 --env production app.psgi"